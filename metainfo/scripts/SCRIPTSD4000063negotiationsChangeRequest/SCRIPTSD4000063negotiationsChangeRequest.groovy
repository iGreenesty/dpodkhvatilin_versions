...
//Параметры-----------------------------------------------------
def LINKED_OBJECTS = 'negotiations'
def CHILD_METACLASS = ''
def CASES = []
def STATES = ['registered']
def STATUS_CONDITION = false
def COUNT_CONDITION = true
def ERROR_MSG = 'В рамках текущего запроса на изменение есть незакрытые согласования'
...
def removeEmptyString(Collection collection) {
  collection.remove('')
  return collection
}
